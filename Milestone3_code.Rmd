...
p[["scatter"]]  <- ggplot(data, aes(x = age, y = thalach)) +
 ...
 p[["scatter2"]] <- ggplot(data, aes(x = age,y = trestbps))+
  geom_point(aes(col=as.factor(target),shape = as.factor(sex))) +
   labs(fill = "Disease Status") +
   stat_smooth(data = data5, aes(x = age, y = trestbps), method = "lm", se = FALSE, col = "red", size = 1.5)+
  stat_smooth(data = data6, method = "lm", se = FALSE, col = "blue", size = 1.5) +
   theme(legend.text = element_text(size = 15))+ 
 ...
 p[["scatter3"]] <- ggplot(data, aes(x = age,y = chol))+
  geom_point(aes(col=as.factor(target),shape = as.factor(sex))) + 
   labs(fill = "Disease Status") + 
   stat_smooth(data = data5, aes(x = age, y = chol), method = "lm", se = FALSE, col = "red", size = 1.5)+
  stat_smooth(data = data6, method = "lm", se = FALSE, col = "blue", size = 1.5) +
   theme(legend.text = element_text(size = 15)) + 
...
server1 <- function(input, output) {
  ...
  output$scatterplot <- renderPlot({scatterplot(heart4, selected())+ ggtitle("Correlation between heart rate and cholestrol")
})
